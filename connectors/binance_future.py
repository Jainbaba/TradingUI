...
class BinanceFutureClient:
    ...
    def get_contracts(self) -> typing.Dict[str, Contracts]:
        ...
        if exchange_info is not None:
            for contract_data in exchange_info["symbols"]:
                contracts[contract_data['symbol']] = Contracts(contract_data)
        return contracts

    def get_historical_candles(self, contract: Contracts, interval: str) -> typing.List[Candle]:
        data = dict()
    ...
    def get_bid_ask(self, contract: Contracts) -> typing.Dict[str, float]:
        data = dict()
        data['symbol'] = contract.symbol
        bid_ask_info = self._make_request('GET', '/fapi/v1/ticker/bookTicker', data)

        if bid_ask_info is not None:
            if contract.symbol not in self.prices:
                self.prices[contract.symbol] = {'bid': float(bid_ask_info["bidPrice"]),
                                                'bidQty': float(bid_ask_info["bidQty"])
                    , 'ask': float(bid_ask_info["askPrice"]), 'askQty': float(bid_ask_info["askQty"])}
            else:
                self.prices[contract.symbol]["bid"] = float(bid_ask_info["bidPrice"])
                self.prices[contract.symbol]['bidQty'] = float(bid_ask_info["bidQty"])
                self.prices[contract.symbol]["ask"] = float(bid_ask_info["askPrice"])
                self.prices[contract.symbol]['askQty'] = float(bid_ask_info["askQty"])
    ...
    def place_order(self, contract: Contracts, side: str, quantity: float, order_type: str, price=None,
    ...
    def cancel_order(self, contract: Contracts, order_id: int) -> OrderStatus:
        data = dict()
        data['symbol'] = contract.symbol
    ...
    def get_order_status(self, contract: Contracts, order_id: int) -> OrderStatus:
        data = dict()
        data['timestamp'] = int(time.time() * 1000)
        data['symbol'] = contract.symbol
        data['orderId'] = order_id
    ...
    def _on_message(self, ms, msg: str):
        data = json.loads(msg)
        if 'e' in data:
